...
def plot_all_metrics(lightning_modules, root_path):
    metrics = [
        ('epoch_training_time', False),
        ('total_training_time', False), 
        ('total_train_loss_epoch', True)
    ]
...
fourier_logger = TensorBoardLogger(save_dir=tensorboard_path, name="fourier")
...
checkpoint_callback = ModelCheckpoint(
    ...
    filename='fourier-{epoch:02d}-{train_loss_epoch:.2f}',
    save_top_k=10,
    verbose=True,
    save_last=True,
...
# trainer_fourier = pl.Trainer(max_epochs=opt.fourier_num_epochs, accelerator='gpu', devices=[1], logger=fourier_logger, log_every_n_steps = 1,  callbacks=[checkpoint_callback]) 
...
def apply_swa(base_model, checkpoint_paths, lr=1e-5):
    # Create the SWA model
    swa_model = AveragedModel(base_model)

    # Load and update with each checkpoint
    for path in checkpoint_paths:
        print(f"Loading checkpoint: {path}")
        state_dict = torch.load(path)
        model = ModelLightningModule(fourier_model, loss_fn, lr, 'fourier')
        model.load_state_dict(state_dict)
        swa_model.update_parameters(model)


    # Create a dummy optimizer for the SWALR scheduler
    optimizer = torch.optim.SGD(swa_model.parameters(), lr=lr)

    # Create the SWA scheduler
    swa_scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lambda epoch: 1)

    return swa_model, optimizer, swa_scheduler

# Define the paths to your checkpoints
root_path = '/home/setareh/alltogether/deepreach/logs/Aug2_small_experiment/checkpoints'
checkpoint_names = [
    'fourier-epoch=10999-train_loss_epoch=63.29.ckpt, 
    'fourier-epoch=11999-train_loss_epoch=74.15.ckpt',
    'fourier-epoch=12999-train_loss_epoch=94.12.ckpt',
    'fourier-epoch=13999-train_loss-epoch=92.35.ckpt'
    # Add more checkpoints as needed
]
checkpoint_paths = [os.path.join(root_path, name) for name in checkpoint_names]

# Apply SWA
swa_model, swa_optimizer, swa_scheduler = apply_swa(fourier_model, checkpoint_paths, lr=opt.lr)

# Create a new LightningModule with the SWA model
swa_lightning_model = ModelLightningModule(swa_model.module, loss_fn, opt.lr, model_name='fourier_swa')

# Create a new trainer for fine-tuning with SWA
swa_trainer = pl.Trainer(
    max_epochs=opt.fourier_num_epochs,
    accelerator='gpu',
    devices=[0],
    logger=fourier_logger,
    log_every_n_steps=1,
    callbacks=[finetuning_checkpoint_callback]
)

# Fine-tune the SWA model
swa_trainer.fit(swa_lightning_model, dataloader)