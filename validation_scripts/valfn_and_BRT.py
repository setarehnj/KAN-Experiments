...
logging_root = './logs/Wedesday24'
os.makedirs(logging_root, exist_ok=True)
angle_alpha = 1.2

# Setting to plot
ckpt_path_mlp = './Deepreach_trained_checkpoints/air3D_ckpt.pth'
ckpt_path_kan = './Deepreach_trained_checkpoints/air3D_ckpt2.pth'  # Add path for second model
activation = 'sine'

def load_model_mlp(ckpt_path):
    model = modules.SingleBVPNetwithDNN(in_features=4, out_features=1, type=activation, mode='mlp',
...
def load_model_kan(ckpt_path):
    model = modules.SingleBVPNetwithKAN(in_features=4, out_features=1,final_layer_factor=1., hidden_features=32, num_hidden_layers=3)
    model.cuda()
    checkpoint = torch.load(ckpt_path)
    try:
        model_weights = checkpoint['model']
...
model_mlp = load_model_mlp(ckpt_path_mlp)
model_kan = load_model_kan(ckpt_path_kan)
