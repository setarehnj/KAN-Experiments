...
chebyshev_logger = TensorBoardLogger(save_dir=tensorboard_path, name="chebyshev")



chebyshev_lightning = ModelLightningModule(chebyshev_model, loss_fn, opt.lr,model_name='chebyshev', baseline_loss=322.0)
...
# Define the trainers
# trainer_dnn = pl.Trainer(max_epochs=opt.dnn_num_epochs, accelerator='gpu',devices = 2, logger= dnn_logger, log_every_n_steps=1)

checkpoint_callback = ModelCheckpoint(
    dirpath=os.path.join(root_path, 'checkpoints'),
    filename='Chebyshev-{epoch:02d}-{train_loss_epoch:.2f}',
    save_top_k=10,
    verbose=True,
    save_last=True,
...
trainer_chebyshev = pl.Trainer(max_epochs=opt.chebyshev_num_epochs, accelerator='gpu', devices=[0], logger=chebyshev_logger, log_every_n_steps=1, callbacks=[checkpoint_callback])
# torch.cuda.empty_cache()
# trainer_fourier = pl.Trainer(max_epochs=opt.fourier_num_epochs, accelerator='gpu', devices=2, logger=fourier_logger, log_every_n_steps=1)
...
#lightning_modules = [dnn_lightning, fourier_lightning]
