...
# p.add_argument('--chebyshev_num_epochs', type=int, default=120000,
#                help='Number of epochs to train for.')
# p.add_argument('--fourier_num_epochs', type=int, default=120000,
#                help='Number of epochs to train for.')
# p.add_argument('--wavelet_num_epochs', type=int, default=120000,
#                help='Number of epochs to train for.')
...
# # Move models to GPU
# chebyshev_model.cuda()
# dnn_model.cuda()
# fourier_model.cuda()
# wavelet_model.cuda()

# # Create a dictionary of models
# models = {'chebyshev': chebyshev_model, 'mlp': dnn_model, 'fourier': fourier_model, 'wavelet': wavelet_model}
...
#     fig_combined = plt.figure(figsize=(5*num_times, 5*num_thetas * 4))  # Increased height for 4 models
...
#                 # Plot the actual data
#                 ax = fig_combined.add_subplot(num_times * 4, num_thetas, (i * 4 + k) * num_thetas + j + 1)
#                 ax.set_title(f'{model_name} - t = {times[i]:0.2f}, theta = {thetas[j]:0.2f}')
#                 s = ax.imshow(model_out.T, cmap='bwr', origin='lower', extent=(-1., 1., -1., 1.))
...
# epochs_dict = {'chebyshev': opt.chebyshev_num_epochs, 'mlp': opt.dnn_num_epochs, 'fourier': opt.fourier_num_epochs, 'wavelet': opt.wavelet_num_epochs}

# # Train the models
# chebyshev_losses, mlp_losses, fourier_losses, wavelet_losses, training_times = training.train(models=models, train_dataloader=dataloader, epochs_dict=epochs_dict, lr=opt.lr,
...
# training.plot_convergence_curves(chebyshev_losses, mlp_losses, fourier_losses, wavelet_losses, root_path)
...
# Create a dictionary of models
models = {'mlp': dnn_model, 'chebyshev': chebyshev_model, 'fourier': fourier_model, 'wavelet': wavelet_model}
