...
# Setting to plot
ckpt_path = '/home/setareh/alltogether/deepreach/logs/experiment_July_28/tensorboard_logs/fourier/version_1/checkpoints/fourier_final.ckpt'
activation = 'sine'
times = [0.9]
...
model = modules.SingleBPVNetWithFourier(in_features=4, out_features=1, hidden_features=40, num_hidden_layers=3)
model.cuda()
...
def val_fn_BRS(model):
  ...
  for i in range(num_times):
    for j in range(num_thetas):
      ...
      boundary_values = torch.norm(coords[:, 1:3], dim=1, keepdim=True) - 0.25
