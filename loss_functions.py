...
def initialize_hji_MultiVehicleCollisionNE(dataset, minWith):
    ...
    def hji_MultiVehicleCollision(model_output, gt):
        ...
        print(f"dirichlet_mask shape: {dirichlet_mask.shape}")
        print(f"source_boundary_values shape: {source_boundary_values.shape}")
...
def initialize_hji_air3D(dataset, minWith):
    ...
    def hji_air3D(model_output, gt):
        source_boundary_values = gt['source_boundary_values']
        #device = model_output['model_out'].device
        #source_boundary_values = gt['source_boundary_values'].to(device)
        #x = model_output['model_in'].to(device)  # (meta_batch_size, num_points, 4)
        #y = model_output['model_out'].to(device) 
        x = model_output['model_in']  # (meta_batch_size, num_points, 4)
        y = model_output['model_out']  # (meta_batch_size, num_points, 1)
        #device = model_output.device
   # y = model_output.to(device)
    #x = gt.to(device)
        dirichlet_mask = gt['dirichlet_mask']
        batch_size = x.shape[1]
